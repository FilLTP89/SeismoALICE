        ...
        if name  is not None:
            ...
            try:
                return type(classname, (BasiceDecoderDataParallele, ), dict(ngpu = ngpu, nz = nz, nch = nch, nly = nly,\
                 ker = ker, std =std, pad = pad, limit=limit, dconv = dconv, path=path,channel = channel,n_extra_layers=n_extra_layers,opd=opd, dil=dil, act=act, bn=bn))
...
class Decoder(BasiceDecoderDataParallele):
    ...
    def __init__(self,ngpu,nz,nch,ndf,nly,channel,act, dconv = "",\
