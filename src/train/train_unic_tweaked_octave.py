...
class trainer(object):
    ...
    def alice_train_discriminator_adv(self,y,zd,x,zf):
        ...
        zdd_gen,zdf_gen = self.F_(y_inp) # zdd_gen = zy, zdf_gen = zyx
        # F(x)|y,F(x)|yx,F(x)|x
        zfd_gen,zff_gen = self.F_(x_inp) # zff_gen = zx, zfd_gen = zxy
 
        zd_gen = zdd_gen # zy generated by y
        
        zf_gen = zff_gen # zx generated by x
        ...
        zdd_gen,zdf_gen = self.F_(y_gen)
        zfd_gen,zff_gen = self.F_(x_gen)

        zd_rec = zdd_gen # zy generated by y
        # zf_rec = zcat(zfd_gen,zff_gen) # zx generated by x
        zf_rec = zff_gen # zx generated by x
    ...
    def alice_train_generator_adv(self,y,zd,x,zf):
       ...
       # pdb.set_trace()
        ...
        zdd_gen,_ = self.F_(y_inp) # zdd_gen = zy, zdf_gen = zyx
        _,zff_gen = self.F_(x_inp) # zff_gen = zx, zfd_gen = zxy
        ...
        zdd_gen,_, = self.F_(y_gen)
        _,zff_gen = self.F_(x_gen)
    ...
    def train_unique(self):
        ...
        for epoch in range(niter):
            ...
            for b,batch in enumerate(trn_loader):
                ...
                # Train G/D
                wnzd = torch.empty(*(opt.batchSize,nzd,opt.latentSize)).normal_(**rndm_args).to(place)
                wnzf = torch.empty(*(opt.batchSize,nzf,opt.latentSize)).normal_(**rndm_args).to(place)
                with torch.cuda.stream(stream):
                    ...
                    for _ in range(1):
                        self.alice_train_generator_adv(y,wnzd,x,wnzf)
                    # torch.cuda.empty_cache()
            
            # GPUtil.showUtilization(all=True)
            
            # for k,v in self.losses.items(): 
            #     self.writer.add_scalar('Loss/{}'.format(k),np.mean(np.array(v[-b:-1])),epoch)

            # self.save_gradient_g_tb(self.F_,"encoder",epoch)
            # self.save_gradient_g_tb(self.Gx,"Gx",epoch)
            # self.save_gradient_g_tb(self.Gy,"Gy",epoch)
