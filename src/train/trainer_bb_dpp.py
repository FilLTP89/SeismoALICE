...
class trainer(object):
    ...
    def __init__(self, gpu, opt):
        ...
        print(f'preparing to train on {rank}')
        print("nccl? ", torch.distributed.is_nccl_available())
        # print("backend ? ".torch.ge)
        ...
        if 'broadband' in t:
            ...
            # if rank == 0:
            self.Fef = net.Encoder(self.opt.config['encoder'],self.opt).to(rank)
            self.Fef = nn.SyncBatchNorm.convert_sync_batchnorm(self.Fef)
            self.Gdf = net.Decoder(self.opt.config['decoder'], self.opt).to(rank)
            self.Gdf = nn.SyncBatchNorm.convert_sync_batchnorm(self.Gdf)
            # dist.barrier(), find_unused_parameters=True

            self.Fef = DDP(self.Fef, device_ids=[rank])
            self.Gdf = DDP(self.Gdf, device_ids=[rank])

            print("encoder and decoder are created")
            # if rank == 0:
            #     print(self.Fef)
            ...
            if self.strategy['tract']['broadband']:
                ...
                self.DsXf = net.DCGAN_Dx(self.opt.config['DsXf']  , self.opt).to(rank)
                self.Dfxz = net.DCGAN_DXZ(self.opt.config['Dfxz'] , self.opt).to(rank)
                # dist.barrier()

                self.Dszf = DDP(self.Dszf, device_ids=[rank], find_unused_parameters=True)
                self.DsXf = DDP(self.DsXf,device_ids=[rank], find_unused_parameters=True)
                self.Dfxz = DDP(self.Dfxz, device_ids=[rank], find_unused_parameters=True)
                ...
                # if rank ==0:
                self.Dsrzf = net.DCGAN_Dz(self.opt.config['Dsrzf'], self.opt).to(rank)
                self.DsrXf = net.DCGAN_Dx(self.opt.config['DsrXf'], self.opt).to(rank)
                # dist.barrier()
...
#OK
        ...
        # loop = tqdm(self.trn_loader,leave = True)
