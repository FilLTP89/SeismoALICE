...
"""
acts={}
acts['ALICE'] = {'Fyx' :[LeakyReLU(1.0,inplace=True) for t in range(nly)]+[LeakyReLU(1.0,inplace=True)],
                 'Gy' :[ReLU(inplace=True) for t in range(nly-1)]+[Tanh()],
                 'Fef' :[LeakyReLU(1.0,inplace=True) for t in range(4)]+[LeakyReLU(1.0,inplace=True)],
                 'Gx' :[ReLU(inplace=True) for t in range(4)]+[Tanh()],
...
acts['WGAN']  = {'Fyx' :[LeakyReLU(1.0,inplace=True) for t in range(nly)]+[LeakyReLU(1.0,inplace=True)],
                 'Gy' :[ReLU(inplace=True) for t in range(nly-1)]+[Tanh()],
                 'Fef' :[LeakyReLU(1.0,inplace=True) for t in range(4)]+[LeakyReLU(1.0,inplace=True)],
                 'Gx' :[ReLU(inplace=True) for t in range(4)]+[Tanh()],
                 'Ghz' :[ReLU(inplace=True) for t in range(2)]+[LeakyReLU(1.0,inplace=True)],
...
class trainer(object):
    ...
    def __init__(self,cv):
        ...
        self.losses = {
            'Gloss':[0],'Dloss':[0],
                       'Gloss_ali_z':[0],'Gloss_cycle_xy':[0],'Gloss_ali_xy':[0],
                       'Gloss_cycle_z':[0],'Dloss_ali':[0],'Dloss_ali_xy':[0],
                       'Dloss_ali_z':[0]}
    # @profile
    ...
    # @profile
    def discriminate_zz(self,z,zr):
        # pdb.set_trace()
        Dreal = self.Dzz(zcat(z,z ))
        Dfake = self.Dzz(zcat(z,zr))
        return Dreal,Dfake
    ...
    def alice_train_discriminator_adv(self,y,zd,x,zf):
        ...
        self.Dyz.train(), self.Dxz.train()
        self.Dyy.train(), self.Dxx.train()
        ...
        Dloss.backward(),self.oDyxz.step(),clipweights(self.Dnets),zerograd(self.optz)
    ...
    def generate(self):
        ...
        if 'unique' in t and self.strategy['trplt']['unique']:
            ...
            plt.plot_generate_classic('broadband',self.F_,self.Gy,device,vtm,\
                                      vld_loader,pfx="vld_set_bb_unique",outf=outf)
        ...
        # if 'filtered' in t and self.strategy['trplt']['filtered']:
            ...
            plt.plot_generate_classic('filtered',self.F_,self.Gx,device,vtm,\
                                      vld_loader,pfx="vld_set_fl_unique",outf=outf)
