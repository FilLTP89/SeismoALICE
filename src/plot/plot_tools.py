...
#             cnt += 1
            ...
            # filtered signals
def plot_distribution(tag,calc,tar, lim=[-5,5],bins = 20,save=False, number=3, echantillons=2):
...
def visualize_signal(opt,Xf,Xr=None, vtm = None, pfx='trial',outf='./imgs',save = False):
    sns.set(style="whitegrid")
    ...
    for (io, ig) in zip(range(Xf.shape[0]),range(Xr.shape[0])):
        ...
        hax0.plot(vtm,gt,color=clr[3],label=r'$G_y(\mathbf{x})$',linewidth=1.2, alpha=0.6)
        hax0.plot(vtm,ot,color=clr[0],label=r'$\mathbf{x}$',linewidth=1.2, alpha=0.9)
        hax1.loglog(vfr,of,color=clr[0],label=r'$\mathbf{x}$',linewidth=2)
        hax1.loglog(vfr,gf,color=clr[3],label=r'$G_y(\mathbf{x})$',linewidth=2)
        hax0.set_xlim(0.0,int(vtm[-1]))
        hax0.set_xticks(np.arange(0.0,int(vtm[-1])*11./10.,int(vtm[-1])/10.))
        hax0.set_ylim(-1.0,1.0)
        hax0.set_yticks(np.arange(-1.0,1.25,0.25))
