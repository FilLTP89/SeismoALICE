...
import random
import pickle 
from CommonTorch import tcuda, tdev, tsave, tload, tFT, tLT, mseed
from CommonTorch import tud_dload, tud_bsmp, tud_rsmp, tdu_cat, cudnn
from common_model import get_truncated_normal
from LoadDatabase import GetMSEEDDataset, GetSabettaPuglieseDataset
from LoadDatabase import GetSTEADDataset, GetANN2BBDataset
import pandas as pd
...
def Parse():
    ...
    parser.add_argument('--nzxy', type=int, default=32, help='size of the latent space')
    parser.add_argument('--nzyy', type=int, default=8, help='size of the latent space')
...
def Setup(opt):
    ...
    if opt.dataset == 'synth':
        ...
        vtm,fsc = GetSabettaPuglieseDataset(opt.batchPercent,opt.imageSize,opt.latentSize,\
        ...
        md['nTn'] = md['vTn'].size
        ths_trn,ths_tst,ths_vld,\
        vtm,fsc = GetSTEADDataset(src,opt.batchPercent,opt.imageSize,opt.latentSize,\
            opt.nzd,opt.nzf,md=md,nsy=opt.nsy,device=device)
        md['fsc']=fsc
        opt.ncls = md['fsc']['ncat']
        # Create natural period vector 
        ...
        md['nTn'] = md['vTn'].size
        ths_trn,ths_tst,ths_vld,\
        vtm,fsc,md = GetANN2BBDataset(src,opt.batchPercent,opt.imageSize,opt.latentSize,\
                                    opt.nzd,opt.nzf,md=md,nsy=opt.nsy,device=device)
        md['fsc']=fsc
        opt.ncls = md['fsc']['ncat']
        # Create natural period vector 
        ...
        ths_trn,ths_tst,ths_vld,thf_trn,thf_tst,thf_vld,\
            vtm,fsc = GetMSEEDDataset(opt.batchPercent,\
            source=src,inventory=inv)

        md = {'mw':[],'dep':[],'scc':[],'sst':[],\
            'dtm':vtm[1]-vtm[0],'ntm':vtm.size,'scl':[]}
        opt.ncls = 1
        tsave(ths_trn,'./ths_trn.pth')
        tsave(ths_tst,'./ths_tst.pth')
        tsave(ths_vld,'./ths_vld.pth')
