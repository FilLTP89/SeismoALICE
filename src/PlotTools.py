...
import os
import numpy as np
from CommonNN import Variable, zcat
from GenerateNoise import latent_resampling, GetNoise
from CommonTorch import tfft, trnd, tfnp, tnan, o0s
from LoadDatabase import arias_intensity
import torch
...
def plot_compare_ann2bb(Qec,Pfc,Qdc,Pdc,Fhz,Ghz,dev,vtm,trn_set,pfx='hybrid',outf='./imgs'):
    ...
    for _,batch in enumerate(trn_set):
        ...
        wnxd,wnzd,_ = noise_generator(Xd.shape,zd.shape,dev,rndm_args)
        wnxf,wnzf,_ = noise_generator(Xf.shape,zf.shape,dev,rndm_args)
        _,wnzc,_ = noise_generator(Xd.shape,zd.shape,dev,rndm_args)
        _,wnzb,_ = noise_generator(Xf.shape,zf.shape,dev,rndm_args)

        Xf_inp = zcat(Xf,wnxf)
        zff = Qec(Xf_inp)
        zdr = Ghz(zcat(zff,wnzb))
        _,wnzd,_ = noise_generator(Xd.shape,zd.shape,dev,rndm_args)
        _,wnzf,_ = noise_generator(Xf.shape,zf.shape,dev,rndm_args)
