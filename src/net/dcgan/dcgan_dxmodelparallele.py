...
class  DCGAN_Dx_1GPU(BasicDCGAN_Dx):
    ...
    def __init__(self, ngpu, nc, ncl, ndf, nly, act, channel, fpd=1, isize=256, limit = 256,\
        ...
        self.prc.append(ConvBlock(ni = channel[0], no = channel[1],
                ker = ker[0], stride = std[0], pad = pad[0], dil = dil[0],\
        ...
        for i in range(2, nly+1):
            ...
            self.net.append(ConvBlock(ni = channel[i-1], no = channel[i],
                ker = ker[i-1], stride = std[i-1], pad = pad[i-1], dil = dil[i-1], bias = False,\
...
class DCGAN_Dx_3GPU(BasicDCGAN_Dx):
    ...
    def forward(self, x):
        ...
        # z = self.cnn3(z)
        x = T._forward_3G(x, self.cnn1, self.cnn2, self.cnn3, self.splits)
        torch.cuda.empty_cache()
