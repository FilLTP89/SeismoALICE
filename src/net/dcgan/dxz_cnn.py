...
class DCGAN_DX(Module):
    def __init__(self, ngpu, dev, isize, nc, ncl, ndf, fpd=0, n_extra_layers=0, dpc=0.0,
                 activation=[LeakyReLU(1.0,True),Sigmoid()],bn=True,wf=False, opt=None):
        super(DCGAN_DX,self).__init__()
        assert isize % 16 == 0, "isize has to be a multiple of 16"
        self.ngpu = ngpu
        ...
        if ngpu ==1:
            ann = initial+extra+pyramid+final#+[Squeeze()]+[Squeeze()]
            self.ann = sqn(*ann)
            self.ann.to(self.dev,non_blocking=True,dtype=torch.float32)
            self.prc.to(self.dev,non_blocking=True,dtype=torch.float32)
            self.exf.to(self.dev,non_blocking=True,dtype=torch.float32)
        else:
            ...
            self.exf.to(self.dev1,non_blocking=True,dtype=torch.float32)

    def extraction(self,X):
        X = self.prc(X).to(torch.float32)
        f = [self.exf[0](X)]
        for l in range(1,len(self.exf)):
            f.append(self.exf[l](f[l-1]))
        if X.is_cuda:
    ...
    def forward(self,X):
        if X.is_cuda:
            if self.ngpu > 1:
                ...
                X = X.to(self.dev)
                z = self.ann(X)
                z = z.to(self.dev,non_blocking=True,dtype=torch.float32)
                return z
            torch.cuda.empty_cache()
        ...
        if self.wf:
            #f = pll(self.extraction,X,self.gang)
            f = self.extraction(X)
            return z.to(torch.float32),f
        else:
            return z.to(torch.float32)
    
class DCGAN_Dz(Module):
    def __init__(self, ngpu, dev, nz, ncl, fpd=0, n_extra_layers=1, dpc=0.0,
                 activation=[LeakyReLU(1.0,True),Sigmoid()],bn=True,wf=False, opt=None):
        super(DCGAN_Dz,self).__init__()
        self.ngpu = ngpu
        self.gang = range(self.ngpu)
...
class DCGAN_DXz(Module):
    def __init__(self, ngpu, dev, nc, n_extra_layers=0, 
                 activation=[LeakyReLU(1.0,inplace=True),LeakyReLU(1.0,inplace=True)],
                 dpc=0.0,wf=False, opt=None):
        super(DCGAN_DXz,self).__init__()
        self.ngpu = ngpu
        ...
        if ngpu==1:
            ...
            self.ann = sqn(*ann)
            self.exf = sqn(*self.exf)
            self.ann.to(dev,non_blocking=True,dtype=torch.float32)
            self.exf.to(self.dev,non_blocking=True,dtype=torch.float32)
        else:
            if ngpu==2:
                self.dev0 = 0
            ...
            self.ann1.to(self.dev0,non_blocking=True,dtype=torch.float32)
            self.ann2.to(self.dev1,non_blocking=True,dtype=torch.float32)
            self.ann3.to(self.dev2,non_blocking=True,dtype=torch.float32)
            self.exf.to(self.dev2,non_blocking=True,dtype=torch.float32)

    def extraction(self,X):
        f = [self.exf[0](X.to(torch.float32))]
        for l in range(1,len(self.exf)):
            f.append(self.exf[l](f[l-1]))
        if X.is_cuda:
            torch.cuda.empty_cache()
