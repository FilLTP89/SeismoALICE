...
class trainer(object):
    ...
    def alice_train_discriminator_adv(self,y,zd,x,zf):
        ...
        zdd_gen,zdf_gen,zdf_ind = self.F_(y_inp) # zdd_gen = zy, zdf_gen = zyx
        zfd_ind,zfd_gen,zff_gen = self.F_(x_inp) # zff_gen = zx, zfd_gen = zxy
        ...
        zdd_gen,zdf_gen,zdf_ind = self.F_(y_gen)
        zfd_ind,zfd_gen,zff_gen = self.F_(x_gen)
    ...
    def alice_train_generator_adv(self,y,zd,x,zf):
       ...
       # pdb.set_trace()
        ...
        Gloss_ali_xy = self.bce_loss(Dfake_y,o1l(Dfake_y))+10*self.bce_loss(Dfake_x,o1l(Dfake_x))
        Gloss_cycle_xy = torch.mean(torch.abs(y-y_rec))+10*torch.mean(torch.abs(x-x_rec))
